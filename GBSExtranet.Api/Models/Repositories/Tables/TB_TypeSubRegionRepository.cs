...
{
    public class TB_TypeSubRegionRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeSubRegionExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeSubRegionExt> list = new List<TB_TypeSubRegionExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeSubRegionExt model = new TB_TypeSubRegionExt();
        ...
        public bool Create(TB_TypeSubRegionExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeSubRegion DepObj = new TB_TypeSubRegion();
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeSubRegion.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeSubRegionExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeSubRegion.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeSubRegionExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeSubRegion.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeSubRegion.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeSubRegionExt
    {
