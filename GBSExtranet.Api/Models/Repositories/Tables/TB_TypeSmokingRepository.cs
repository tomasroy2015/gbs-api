...
{
    public class TB_TypeSmokingRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeSmokingExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeSmokingExt> list = new List<TB_TypeSmokingExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeSmokingExt model = new TB_TypeSmokingExt();
        ...
        public bool Create(TB_TypeSmokingExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeSmoking DepObj = new TB_TypeSmoking();
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeSmoking.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeSmokingExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeSmoking.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeSmokingExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeSmoking.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeSmoking.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeSmokingExt
    {
