...
{
    public class TB_TypePricePolicyRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypePricePolicyExt> GetAllTableValue(int TableID)
        {
            List<TB_TypePricePolicyExt> list = new List<TB_TypePricePolicyExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypePricePolicyExt model = new TB_TypePricePolicyExt();
        ...
        public bool Create(TB_TypePricePolicyExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypePricePolicy DepObj = new TB_TypePricePolicy();
           // DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypePricePolicy.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypePricePolicyExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypePricePolicy.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypePricePolicyExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypePricePolicy.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypePricePolicy.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypePricePolicyExt
    {
