...
{
    public class TB_TypeTravellerRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeTravellerExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeTravellerExt> list = new List<TB_TypeTravellerExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeTravellerExt model = new TB_TypeTravellerExt();
        ...
        public bool Create(TB_TypeTravellerExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeTraveller DepObj = new TB_TypeTraveller();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeTraveller.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeTravellerExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeTraveller.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeTravellerExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeTraveller.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeTraveller.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeTravellerExt
    {
