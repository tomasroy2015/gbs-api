...
{
    public class TB_TypeStatusRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeStatusExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeStatusExt> list = new List<TB_TypeStatusExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeStatusExt model = new TB_TypeStatusExt();
        ...
        public bool Create(TB_TypeStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeStatus DepObj = new TB_TypeStatus();
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeStatus.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeStatus.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
