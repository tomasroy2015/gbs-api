...
{
    public class TB_TypeSalutationRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeSalutationExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeSalutationExt> list = new List<TB_TypeSalutationExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeSalutationExt model = new TB_TypeSalutationExt();
        ...
        public bool Update(TB_TypeSalutationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeSalutation.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeSalutationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeSalutation.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeSalutation.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeSalutationExt
    {
