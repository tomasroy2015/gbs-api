...
{
    public class TB_TypeTourFrequencyRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeTourFrequencyExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeTourFrequencyExt> list = new List<TB_TypeTourFrequencyExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeTourFrequencyExt model = new TB_TypeTourFrequencyExt();
                    ...
                    model.Period = Convert.ToBoolean(dr["HasPeriod"]);
        ...
        public bool Create(TB_TypeTourFrequencyExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeTourFrequency DepObj = new TB_TypeTourFrequency();
            ...
            DepObj.HasPeriod = model.Period;
            DepObj.Sort = Convert.ToInt16(model.Sorts);
            DepObj.Active = model.Active;
            DepObj.OpDateTime = DateTime.Now;
            DepObj.OpUserID = 0;
            insertentity.TB_TypeTourFrequency.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeTourFrequencyExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeTourFrequency.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeTourFrequencyExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeTourFrequency.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeTourFrequency.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeTourFrequencyExt
    {
