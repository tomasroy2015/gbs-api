...
{
    public class TB_TypeOperationRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeOperationExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeOperationExt> list = new List<TB_TypeOperationExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeOperationExt model = new TB_TypeOperationExt();
        ...
        public bool Create(TB_TypeOperationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeOperation DepObj = new TB_TypeOperation();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeOperation.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeOperationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeOperation.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeOperationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeOperation.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeOperation.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeOperationExt
    {
