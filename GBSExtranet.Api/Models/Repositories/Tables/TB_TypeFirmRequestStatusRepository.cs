...
{
    public class TB_TypeFirmRequestStatusRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeFirmRequestStatusExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeFirmRequestStatusExt> list = new List<TB_TypeFirmRequestStatusExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeFirmRequestStatusExt model = new TB_TypeFirmRequestStatusExt();
        ...
        public bool Create(TB_TypeFirmRequestStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeFirmRequestStatus DepObj = new TB_TypeFirmRequestStatus();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeFirmRequestStatus.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeFirmRequestStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeFirmRequestStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeFirmRequestStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeFirmRequestStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeFirmRequestStatus.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeFirmRequestStatusExt
    {
