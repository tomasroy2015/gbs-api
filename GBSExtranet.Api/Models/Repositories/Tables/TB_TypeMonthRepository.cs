...
{
    public class TB_TypeMonthRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeMonthExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeMonthExt> list = new List<TB_TypeMonthExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeMonthExt model = new TB_TypeMonthExt();
        ...
        public bool Create(TB_TypeMonthExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeMonth DepObj = new TB_TypeMonth();
            //DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeMonth.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeMonthExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeMonth.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeMonthExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeMonth.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeMonth.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
