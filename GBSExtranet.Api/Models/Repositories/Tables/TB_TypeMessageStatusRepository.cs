...
{
    public class TB_TypeMessageStatusRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeMessageStatusExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeMessageStatusExt> list = new List<TB_TypeMessageStatusExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeMessageStatusExt model = new TB_TypeMessageStatusExt();
        ...
        public bool Create(TB_TypeMessageStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeMessageStatus DepObj = new TB_TypeMessageStatus();
            //DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeMessageStatus.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeMessageStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeMessageStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeMessageStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeMessageStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeMessageStatus.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeMessageStatusExt
    {
