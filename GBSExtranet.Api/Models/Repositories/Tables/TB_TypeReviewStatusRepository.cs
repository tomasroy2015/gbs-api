...
{
    public class TB_TypeReviewStatusRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeReviewStatusExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeReviewStatusExt> list = new List<TB_TypeReviewStatusExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeReviewStatusExt model = new TB_TypeReviewStatusExt();
        ...
        public bool Create(TB_TypeReviewStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeReviewStatus DepObj = new TB_TypeReviewStatus();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeReviewStatus.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeReviewStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReviewStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeReviewStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReviewStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeReviewStatus.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeReviewStatusExt
    {
