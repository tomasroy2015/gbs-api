...
{
    public class TB_TypeHotelAccommodationRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeHotelAccommodationExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeHotelAccommodationExt> list = new List<TB_TypeHotelAccommodationExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeHotelAccommodationExt model = new TB_TypeHotelAccommodationExt();
        ...
        public bool Create(TB_TypeHotelAccommodationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeHotelAccommodation DepObj = new TB_TypeHotelAccommodation();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeHotelAccommodation.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeHotelAccommodationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeHotelAccommodation.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeHotelAccommodationExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeHotelAccommodation.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeHotelAccommodation.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeHotelAccommodationExt
    {
