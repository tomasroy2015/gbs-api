...
{
    public class TB_TypeBusinessPartnerRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeBusinessPartnerExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeBusinessPartnerExt> list = new List<TB_TypeBusinessPartnerExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeBusinessPartnerExt model = new TB_TypeBusinessPartnerExt();
        ...
        public bool Create(TB_TypeBusinessPartnerExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeBusinessPartner DepObj = new TB_TypeBusinessPartner();
            //DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeBusinessPartner.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeBusinessPartnerExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeBusinessPartner.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeBusinessPartnerExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeBusinessPartner.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeBusinessPartner.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeBusinessPartnerExt
    {
