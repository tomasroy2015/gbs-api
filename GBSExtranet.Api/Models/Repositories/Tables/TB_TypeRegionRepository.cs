...
{
    public class TB_TypeRegionRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeRegionExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeRegionExt> list = new List<TB_TypeRegionExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeRegionExt model = new TB_TypeRegionExt();
        ...
        public bool Create(TB_TypeRegionExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeRegion DepObj = new TB_TypeRegion();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeRegion.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeRegionExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeRegion.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeRegionExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeRegion.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeRegion.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
