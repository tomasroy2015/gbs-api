...
{
    public class TB_TypeViewRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeViewExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeViewExt> list = new List<TB_TypeViewExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeViewExt model = new TB_TypeViewExt();
                    ...
                    model.IncludeCaption = Convert.ToBoolean(dr["IncludedInRoomTypeCaption"]);
        ...
        public bool Create(TB_TypeViewExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeView DepObj = new TB_TypeView();
            ...
            DepObj.IncludedInRoomTypeCaption = model.IncludeCaption;
            DepObj.Sort = Convert.ToInt16(model.Sorts);
            DepObj.Active = model.Active;
            DepObj.OpDateTime = DateTime.Now;
            DepObj.OpUserID = 0;
            insertentity.TB_TypeView.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeViewExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeView.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeViewExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeView.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeView.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
