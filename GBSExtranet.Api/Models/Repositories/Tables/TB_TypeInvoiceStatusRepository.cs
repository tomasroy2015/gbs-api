...
{
    public class TB_TypeInvoiceStatusRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeInvoiceStatusExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeInvoiceStatusExt> list = new List<TB_TypeInvoiceStatusExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeInvoiceStatusExt model = new TB_TypeInvoiceStatusExt();
        ...
        public bool Create(TB_TypeInvoiceStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeInvoiceStatus DepObj = new TB_TypeInvoiceStatus();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeInvoiceStatus.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeInvoiceStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeInvoiceStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeInvoiceStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeInvoiceStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeInvoiceStatus.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeInvoiceStatusExt
    {
