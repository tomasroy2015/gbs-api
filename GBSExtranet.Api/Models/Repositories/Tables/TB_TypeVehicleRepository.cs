...
{
    public class TB_TypeVehicleRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeVehicleExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeVehicleExt> list = new List<TB_TypeVehicleExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeVehicleExt model = new TB_TypeVehicleExt();
        ...
        public bool Create(TB_TypeVehicleExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeVehicle DepObj = new TB_TypeVehicle();
          //  DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeVehicle.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeVehicleExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeVehicle.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeVehicleExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeVehicle.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeVehicle.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeVehicleExt
    {
