...
{
    public class TB_TypeReservationStatusRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeReservationStatusExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeReservationStatusExt> list = new List<TB_TypeReservationStatusExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeReservationStatusExt model = new TB_TypeReservationStatusExt();
        ...
        public bool Create(TB_TypeReservationStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeReservationStatus DepObj = new TB_TypeReservationStatus();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeReservationStatus.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeReservationStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReservationStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.PartID = model.PartID;
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
        ...
        public bool Delete(TB_TypeReservationStatusExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReservationStatus.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeReservationStatus.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeReservationStatusExt
    {
