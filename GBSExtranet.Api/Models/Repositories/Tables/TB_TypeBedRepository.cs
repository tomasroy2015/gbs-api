...
{
    public class TB_TypeBedRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeBedExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeBedExt> list = new List<TB_TypeBedExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeBedExt model = new TB_TypeBedExt();
        ...
        public bool Create(TB_TypeBedExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeBed DepObj = new TB_TypeBed();
            //DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeBed.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeBedExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeBed.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeBedExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeBed.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeBed.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
