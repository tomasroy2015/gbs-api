...
{
    public class TB_TypeFreebieRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeFreebieExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeFreebieExt> list = new List<TB_TypeFreebieExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeFreebieExt model = new TB_TypeFreebieExt();
        ...
        public bool Create(TB_TypeFreebieExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeFreebie DepObj = new TB_TypeFreebie();
            //DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeFreebie.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeFreebieExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeFreebie.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeFreebieExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeFreebie.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeFreebie.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeFreebieExt
    {
