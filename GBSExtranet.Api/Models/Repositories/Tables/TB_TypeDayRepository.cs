...
{
    public class TB_TypeDayRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeDayExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeDayExt> list = new List<TB_TypeDayExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeDayExt model = new TB_TypeDayExt();
                    ...
                    model.WeekDay = Convert.ToInt32(dr["WeekDay"]);
        ...
        public bool Create(TB_TypeDayExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeDay DepObj = new TB_TypeDay();
            //DepObj.ID = model.ID;
            ...
            DepObj.WeekDay = model.WeekDay;
            DepObj.Active = model.Active;

            DepObj.OpDateTime = DateTime.Now;
            DepObj.OpUserID = 0;
            insertentity.TB_TypeDay.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeDayExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeDay.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeDayExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeDay.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeDay.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
