...
{
    public class TB_TypeMessageSubjectRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeMessageSubjectExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeMessageSubjectExt> list = new List<TB_TypeMessageSubjectExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeMessageSubjectExt model = new TB_TypeMessageSubjectExt();
        ...
        public bool Create(TB_TypeMessageSubjectExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeMessageSubject DepObj = new TB_TypeMessageSubject();
            //DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeMessageSubject.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeMessageSubjectExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeMessageSubject.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeMessageSubjectExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeMessageSubject.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeMessageSubject.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeMessageSubjectExt
    {
