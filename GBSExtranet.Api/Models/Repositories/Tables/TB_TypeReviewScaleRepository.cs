...
{
    public class TB_TypeReviewScaleRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeReviewScaleExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeReviewScaleExt> list = new List<TB_TypeReviewScaleExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeReviewScaleExt model = new TB_TypeReviewScaleExt();
                    ...
                    model.Points = Convert.ToDecimal(dr["Point"]);
        ...
        public bool Update(TB_TypeReviewScaleExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReviewScale.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeReviewScaleExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReviewScale.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeReviewScale.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeReviewScaleExt
    {
