...
{
    public class TB_TypeRoomRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeRoomExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeRoomExt> list = new List<TB_TypeRoomExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeRoomExt model = new TB_TypeRoomExt();
        ...
        public bool Create(TB_TypeRoomExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeRoom DepObj = new TB_TypeRoom();
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeRoom.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeRoomExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeRoom.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeRoomExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeRoom.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeRoom.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
