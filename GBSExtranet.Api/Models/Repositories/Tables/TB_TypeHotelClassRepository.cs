...
{
    public class TB_TypeHotelClassRepository:BaseRepository
    {

        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeHotelClassExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeHotelClassExt> list = new List<TB_TypeHotelClassExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeHotelClassExt model = new TB_TypeHotelClassExt();
        ...
        public bool Delete(TB_TypeHotelClassExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeHotelClass.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeHotelClass.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypeHotelClassExt
    {
