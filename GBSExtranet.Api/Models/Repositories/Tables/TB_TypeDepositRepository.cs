...
{
    public class TB_TypeDepositRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeDepositExt> GetAllTypeDeposit(int TableID)
        {
            List<TB_TypeDepositExt> list = new List<TB_TypeDepositExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeDepositExt model = new TB_TypeDepositExt();
        ...
        public bool Create(TB_TypeDepositExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeDeposit DepObj = new TB_TypeDeposit();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeDeposit.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeDepositExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeDeposit.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
                DepObj.Name_es = model.Name_es;
        ...
        public bool Delete(TB_TypeDepositExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeDeposit.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeDeposit.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
        ...
        public class TB_TypeDepositExt
        {
