...
{
    public class TB_TypePenaltyRateRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypePenaltyRateExt> GetAllTableValue(int TableID)
        {
            List<TB_TypePenaltyRateExt> list = new List<TB_TypePenaltyRateExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypePenaltyRateExt model = new TB_TypePenaltyRateExt();
        ...
        public bool Create(TB_TypePenaltyRateExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypePenaltyRate DepObj = new TB_TypePenaltyRate();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypePenaltyRate.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypePenaltyRateExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypePenaltyRate.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.Name_en = model.Name_en;
                DepObj.PartID = model.PartID;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
        ...
        public bool Delete(TB_TypePenaltyRateExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypePenaltyRate.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypePenaltyRate.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
    ...
    public class TB_TypePenaltyRateExt
    {
