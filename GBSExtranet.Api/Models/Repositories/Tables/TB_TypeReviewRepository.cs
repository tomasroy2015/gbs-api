...
{
    public class TB_TypeReviewRepository:BaseRepository
    {
        public  string CultureCode = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
        public List<TB_TypeReviewExt> GetAllTableValue(int TableID)
        {
            List<TB_TypeReviewExt> list = new List<TB_TypeReviewExt>();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TB_TypeReviewExt model = new TB_TypeReviewExt();
        ...
        public bool Create(TB_TypeReviewExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            DBEntities insertentity = new DBEntities();
            TB_TypeReview DepObj = new TB_TypeReview();
            DepObj.ID = model.ID;
            ...
            DepObj.OpUserID = 0;
            insertentity.TB_TypeReview.Add(DepObj);
            insertentity.SaveChanges();
            int id = DepObj.ID;
            return status;
        }

        public bool Update(TB_TypeReviewExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReview.Where(x => x.ID == model.ID).FirstOrDefault();
                DepObj.PartID = model.PartID;
                DepObj.Name_en = model.Name_en;
                DepObj.Name_tr = model.Name_tr;
                DepObj.Name_de = model.Name_de;
        ...
        public bool Delete(TB_TypeReviewExt model, ref string Msg, Controller ctrl)
        {
            bool status = true;
            using (DBEntities DE = new DBEntities())
            {
                var DepObj = DE.TB_TypeReview.Where(x => x.ID == model.ID).FirstOrDefault();
                DE.TB_TypeReview.Remove(DepObj);
                DE.SaveChanges();
            }
            return status;
